...
WEB_PANEL_HEARTBEAT_URL = f"{WEB_PANEL_BASE_URL}/heartbeat"
WEB_PANEL_LOGIN_URL = f"{WEB_PANEL_BASE_URL}/login"
WEB_PANEL_REGISTER_URL = f"{WEB_PANEL_BASE_URL}/register"
WEB_PANEL_PROFILE_URL = f"{WEB_PANEL_BASE_URL}/profile"
WEB_PANEL_LOGOUT_URL = f"{WEB_PANEL_BASE_URL}/logout"
# WEB_PANEL_BASE_URL = "http://localhost:8000/api"
...
class ModernRemoteServerApp(ctk.CTk):
    def __init__(self):
        ...
        # Widget referensi yang dibuat dinamis (akun)
        self.account_container = None
        self._acc_email_entry = None
        self._acc_pass_entry = None
        self._reg_name_entry = None
        self._reg_email_entry = None
        self._reg_pass_entry = None
        self._reg_confirm_entry = None
        self._acc_feedback_label = None
    ...
    def setup_ui(self):
        ...
        self.tab_account = self.tabview.add("Akun")
    ...
    # ---------------------------------------------------------------
    # TAB AKUN (LOGIN / REGISTER / PROFILE)
    # ---------------------------------------------------------------
    def setup_account_tab(self):
        self.account_container = ctk.CTkFrame(self.tab_account, fg_color="transparent")
        self.account_container.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def refresh_account_view(self):
        ...
        if self.config_data.get("auth_token"):
            self.build_profile_view(self.account_container)
        else:
            self.build_login_view(self.account_container)

    def _account_feedback(self, parent):
        label = ctk.CTkLabel(parent, text="", font=ctk.CTkFont(size=11), wraplength=380)
        label.pack(pady=(2, 6))
        return label

    def build_login_view(self, parent):
        ctk.CTkLabel(parent, text="Masuk ke Akun", font=ctk.CTkFont(size=15, weight="bold")).pack(pady=(10, 10))

        ctk.CTkLabel(parent, text="Email:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._acc_email_entry = ctk.CTkEntry(parent, placeholder_text="nama@email.com")
        self._acc_email_entry.pack(fill="x", padx=10, pady=(0, 8))

        ctk.CTkLabel(parent, text="Password:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._acc_pass_entry = ctk.CTkEntry(parent, show="*")
        self._acc_pass_entry.pack(fill="x", padx=10, pady=(0, 4))

        self._acc_feedback_label = self._account_feedback(parent)

        self.login_btn = ctk.CTkButton(
            parent, text="Login", command=self.on_login_submit,
            fg_color="#3b82f6", hover_color="#2563eb"
        )
        self.login_btn.pack(fill="x", padx=10, pady=(4, 4))

        ctk.CTkButton(
            parent, text="Belum punya akun? Daftar di sini", command=lambda: self.build_register_view(self.account_container),
    ...
    def build_register_view(self, parent):
        self._clear_account_container()
        ctk.CTkLabel(parent, text="Buat Akun Baru", font=ctk.CTkFont(size=15, weight="bold")).pack(pady=(10, 10))

        ctk.CTkLabel(parent, text="Nama:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._reg_name_entry = ctk.CTkEntry(parent, placeholder_text="Nama Lengkap")
        self._reg_name_entry.pack(fill="x", padx=10, pady=(0, 8))

        ctk.CTkLabel(parent, text="Email:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._reg_email_entry = ctk.CTkEntry(parent, placeholder_text="nama@email.com")
        self._reg_email_entry.pack(fill="x", padx=10, pady=(0, 8))

        ctk.CTkLabel(parent, text="Password:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._reg_pass_entry = ctk.CTkEntry(parent, show="*")
        self._reg_pass_entry.pack(fill="x", padx=10, pady=(0, 8))

        ctk.CTkLabel(parent, text="Konfirmasi Password:", font=ctk.CTkFont(size=12)).pack(anchor="w", padx=10)
        self._reg_confirm_entry = ctk.CTkEntry(parent, show="*")
        self._reg_confirm_entry.pack(fill="x", padx=10, pady=(0, 4))

        self._acc_feedback_label = self._account_feedback(parent)

        self.register_btn = ctk.CTkButton(
            parent, text="Daftar", command=self.on_register_submit,
            fg_color="#10b981", hover_color="#059669"
        )
        self.register_btn.pack(fill="x", padx=10, pady=(4, 4))

        ctk.CTkButton(
            parent, text="Sudah punya akun? Login", command=lambda: self.refresh_account_view(),
            fg_color="transparent", hover_color="#2b2b2b", text_color="#3b82f6"
        ).pack(fill="x", padx=10, pady=(2, 10))

    def build_profile_view(self, parent):
        ctk.CTkLabel(parent, text="Profil Akun", font=ctk.CTkFont(size=15, weight="bold")).pack(pady=(10, 10))

        card = ctk.CTkFrame(parent, fg_color="#2b2b2b", corner_radius=8)
        card.pack(fill="x", padx=10, pady=(0, 10))

        name = self.config_data.get("user_name") or "-"
        email = self.config_data.get("user_email") or "-"

        ctk.CTkLabel(card, text=name, font=ctk.CTkFont(size=16, weight="bold")).pack(anchor="w", padx=12, pady=(10, 0))
        ctk.CTkLabel(card, text=email, font=ctk.CTkFont(size=12), text_color="#9ca3af").pack(anchor="w", padx=12, pady=(0, 10))

        detail_frame = ctk.CTkFrame(parent, fg_color="#232323", corner_radius=8)
        detail_frame.pack(fill="x", padx=10, pady=(0, 10))

        ctk.CTkLabel(detail_frame, text="Device ID:", font=ctk.CTkFont(size=11), text_color="#9ca3af").pack(anchor="w", padx=12, pady=(10, 0))
        ctk.CTkLabel(detail_frame, text=self.device_id, font=ctk.CTkFont(size=11, family="Consolas")).pack(anchor="w", padx=12, pady=(0, 10))

        self._acc_feedback_label = self._account_feedback(parent)

        self.logout_btn = ctk.CTkButton(
            parent, text="Logout", command=self.on_logout,
            fg_color="#ef4444", hover_color="#dc2626"
        )
        self.logout_btn.pack(fill="x", padx=10, pady=(4, 10))

    # --- Aksi akun ---
    def on_login_submit(self):
        email = self._acc_email_entry.get().strip()
        password = self._acc_pass_entry.get()

        if not email or not EMAIL_RE.match(email):
            self._acc_feedback_label.configure(text="Email tidak valid.", text_color="#ef4444")
            return
        if not password:
            self._acc_feedback_label.configure(text="Password wajib diisi.", text_color="#ef4444")
            return

        self.login_btn.configure(text="Memproses...", state="disabled")
        self._acc_feedback_label.configure(text="", text_color="#9ca3af")
        threading.Thread(target=self._login_worker, args=(email, password), daemon=True).start()

    def _login_worker(self, email, password):
        try:
            res_data = api_post(WEB_PANEL_LOGIN_URL, {
                "email": email,
                "password": password,
            ...
            if res_data.get("success"):
                ...
                self.config_data["auth_token"] = token
                self.config_data["user_name"] = user.get("name", "")
                self.config_data["user_email"] = user.get("email", email)
                save_config(self.config_data)

                self.after(0, self.refresh_account_view)
            else:
                msg = res_data.get("message", "Email atau password salah.")
                self.after(0, lambda: self._acc_feedback_label.configure(text=msg, text_color="#ef4444"))
                self.after(0, lambda: self.login_btn.configure(text="Login", state="normal"))
        except (urllib.error.URLError, socket.timeout):
            self.after(0, lambda: self._acc_feedback_label.configure(text="Gagal terhubung ke server.", text_color="#ef4444"))
            self.after(0, lambda: self.login_btn.configure(text="Login", state="normal"))
        except Exception as e:
            print(f"Login error: {e}")
            self.after(0, lambda: self._acc_feedback_label.configure(text="Terjadi kesalahan saat login.", text_color="#ef4444"))
            self.after(0, lambda: self.login_btn.configure(text="Login", state="normal"))

    def on_register_submit(self):
        name = self._reg_name_entry.get().strip()
        email = self._reg_email_entry.get().strip()
        password = self._reg_pass_entry.get()
        confirm = self._reg_confirm_entry.get()

        if not name:
            self._acc_feedback_label.configure(text="Nama wajib diisi.", text_color="#ef4444")
            return
        if not email or not EMAIL_RE.match(email):
            self._acc_feedback_label.configure(text="Email tidak valid.", text_color="#ef4444")
            return
        if len(password) < 6:
            self._acc_feedback_label.configure(text="Password minimal 6 karakter.", text_color="#ef4444")
            return
        if password != confirm:
            self._acc_feedback_label.configure(text="Konfirmasi password tidak cocok.", text_color="#ef4444")
            return

        self.register_btn.configure(text="Memproses...", state="disabled")
        self._acc_feedback_label.configure(text="", text_color="#9ca3af")
        threading.Thread(target=self._register_worker, args=(name, email, password), daemon=True).start()

    def _register_worker(self, name, email, password):
        try:
            res_data = api_post(WEB_PANEL_REGISTER_URL, {
                "name": name,
                "email": email,
                "password": password,
                "device_id": self.device_id,
            ...
            if res_data.get("success"):
                self.after(0, lambda: self._acc_feedback_label.configure(
                    text="Registrasi berhasil! Silakan login.", text_color="#10b981"
                ))
                self.after(800, self.refresh_account_view)
            else:
                msg = res_data.get("message", "Registrasi gagal. Coba email lain.")
                self.after(0, lambda: self._acc_feedback_label.configure(text=msg, text_color="#ef4444"))
                self.after(0, lambda: self.register_btn.configure(text="Daftar", state="normal"))
        except (urllib.error.URLError, socket.timeout):
            self.after(0, lambda: self._acc_feedback_label.configure(text="Gagal terhubung ke server.", text_color="#ef4444"))
            self.after(0, lambda: self.register_btn.configure(text="Daftar", state="normal"))
        except Exception as e:
            print(f"Register error: {e}")
            self.after(0, lambda: self._acc_feedback_label.configure(text="Terjadi kesalahan saat mendaftar.", text_color="#ef4444"))
            self.after(0, lambda: self.register_btn.configure(text="Daftar", state="normal"))

    def on_logout(self):
        token = self.config_data.get("auth_token", "")
        self.config_data["auth_token"] = ""
        self.config_data["user_name"] = ""
        self.config_data["user_email"] = ""
        save_config(self.config_data)
        self.refresh_account_view()
        if token:
            threading.Thread(target=self._logout_worker, args=(token,), daemon=True).start()

    def _logout_worker(self, token):
        try:
            api_post(WEB_PANEL_LOGOUT_URL, {"token": token, "device_id": self.device_id}, timeout=5)
        except Exception:
            pass  # Logout lokal tetap berhasil walau request ke server gagal
    ...
    def _heartbeat_worker(self):
        ...
        while self.is_running:
            try:
                api_post(WEB_PANEL_HEARTBEAT_URL, {
                    "device_id": self.device_id,
                    "web_token": self.config_data.get("web_token", ""),
                    "auth_token": self.config_data.get("auth_token", ""),
                    "mode": self.config_data.get("mode", "Lokal (LAN)"),
                    "port": self.port,
                ...
                pass  # Gagal ping (offline dsb), abaikan tanpa mengganggu program
    ...
    # --- 7. LOGIKA WEBSOCKET SERVER (KEYBOARD + MOUSE + STREAM) ---
    async def websocket_handler(self, websocket):
        ...
        # Sesi per-koneksi: dipakai untuk kontrol streaming layar
        ...
        try:
            async for message in websocket:
                ...
                elif cmd_type in ("mouse_move", "mouse_click", "mouse_scroll", "mouse_position"):
    ...
    def process_mouse_command(self, data):
        ...
        try:
            ...
            if cmd_type == "mouse_move":
                # Gerakan relatif (delta) dari trackpad Android
                dx = float(data.get("dx", 0))
                dy = float(data.get("dy", 0))
                mouse.move(dx, dy)

            elif cmd_type == "mouse_position":
                # Posisi absolut, misal dari tap-to-move
                x = int(data.get("x", 0))
                y = int(data.get("y", 0))
                mouse.position = (x, y)
    ...
    async def stream_screen(self, websocket, session):
        """Loop pengiriman frame layar (JPEG base64) selama sesi streaming aktif."""
        fps = max(1, int(self.config_data.get("stream_fps", 15)))
        quality = max(10, min(95, int(self.config_data.get("stream_quality", 50))))
        delay = 1.0 / fps
